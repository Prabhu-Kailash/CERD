...
const filterByRTE = document.getElementById("filterByRTE");
let current_page = 1;
const rows = 10;
        ...
        let item_head = `<th scope="row">${item.admissionNumber}</th>
            ...
            <td><form action="/staff/transfer/${item._id}/?_method=PATCH" method="POST" style="display: inline;">
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-body">
                        <h4>Do you want to terminate/transfer the staff?</h4>
                    </div>
                    <div class="modal-footer">
...
function displayList (items, wrapper, rows_per_page, page) {
    ...
    for (let i = 0; i < paginatedItems.length; i++) {
        ...
        let item = paginatedItems[i];
        let item_element = document.createElement("tr");
        let item_head = `<th scope="row"><a href="/Revenue/${item._id}" style="color: darkred; text-decoration: none;">${item.classes.admissionNumber}</a></th>
            <th scope="row"><a href="/Report/${item._id}" style="color: black; text-decoration: none;">${item.name}</a></th>
            <td>${item.classes.class}</td>
            <td>${item.classes.section}</td>
            <td>${item.classes.DOJ}</td>
            <td>${item.classes.RTE}</td>
            <td><a href="/students/edit/${item._id}/"><button class="btn btn-success mr-5">Update</button></a>
            <form action="/students/delete/${item._id}/?_method=DELETE" method="POST" style="display: inline;">
                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#Modal${i}">Delete</button>
                <!-- Modal -->
            ...
            <td><form action="/transfer/${item._id}/?_method=PATCH" method="POST" style="display: inline;">
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#TModal${i}">Transfer</button>
                <!-- Modal -->
                <div class="modal fade" id="TModal${i}" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="ModalLabel">Confirm!</h5>
